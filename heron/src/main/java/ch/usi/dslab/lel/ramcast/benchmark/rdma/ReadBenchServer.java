...
public class ReadBenchServer implements RdmaEndpointFactory<ReadBenchServer.ReadServerEndpoint> {
  ...
  public ReadBenchServer(String host, int port, int size) throws IOException {
    this.group = new RdmaActiveEndpointGroup<ReadServerEndpoint>(1, false, 128, 4, 128);
  ...
  public static void main(String[] args) throws Exception {
    ...
    ReadBenchServer server = new ReadBenchServer(options.getServerAddress(),
            options.getServerPort(), options.getPayloadSize());
    server.run();
  }

  public ReadBenchServer.ReadServerEndpoint createEndpoint(RdmaCmId id, boolean serverSide)
          throws IOException {
    return new ReadServerEndpoint(group, id, serverSide, size);
  }

  private void run() throws Exception {
    System.out.println("ReadServer, size " + size);
    ...
    ReadBenchServer.ReadServerEndpoint endpoint = serverEndpoint.accept();
